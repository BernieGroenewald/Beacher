    ...
    public class DBStuff : MarshalByRefObject, IDisposable
    {
        ...
        public SqlConnection ConnectSQL()
        {
            try
            {
                ...
                {
                    _ConnectionString = GlobalStatic.ConnectionString;
                }
        ...
        private static void AssignParameterValues(SqlParameter[] commandParameters, object[] parameterValues)
        {
            ...
            for (int i = 0, j = commandParameters.Length; i < j; i++)
            {
                ...
                if (parameterValues[i] is IDbDataParameter)
                {
                    ...
                    {
                        if (commandParameters[i].SqlDbType == SqlDbType.UniqueIdentifier)
                        {
                            commandParameters[i].Value = Guid.NewGuid();
                        }
                        else
                        {
                ...
                {
                    if (parameterValues[i].ToString().Trim() == "")
                    {
                        if (commandParameters[i].IsNullable)
                        {
                            commandParameters[i].Value = DBNull.Value;
                        }
                        else
                        {
                            switch (commandParameters[i].DbType)
                            {
